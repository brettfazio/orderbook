...
int main()
{
  ...
  // std::ofstream output;
  // output.open("parsed_feed.txt");

  // for (auto item : raw_feed)
  // {
  //   unsigned short mult = static_cast<unsigned short>(item.price * 100);
  //   // {"SYM", "ID8", 0, 47.99, 500},
  //   output << "{\"" << item.symbol << "\", \"" << item.trader;
  //   output << "\", " << item.side << ", " << mult;
  //   output << ", " << item.size << "},\n";
  // }
  // output.close();
